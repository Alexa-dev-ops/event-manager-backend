      ...
      (err) => {
        if (err) {
          ...
          if (retries > 0) {
            ...
            setTimeout(() => {
              connectWithRetry(retries - 1).then(resolve).catch(reject);
            }, 1000);
          } else {
            reject(err);
          }
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        setTimeout(() => {
          if (!hasError && tablesCreated === totalTables) {
        ...
        }, 500); // Small delay to ensure tables are created first
      });
    });
  } catch (error) {
...
// Export the database instance (async initialization)
let dbInstance = null;
